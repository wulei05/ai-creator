    ...
    <div className="max-w-5xl mx-auto space-y-6 md:space-y-8">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-sm">AI 工具</h2>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          ].map(({ icon: Icon, label, desc, gradient, soon }) => (
            <button
              key={label}
              disabled={soon}
              onClick={() => !soon && toast.info(`${label} 功能即将上线`)}
              className="group relative overflow-hidden rounded-xl border bg-card text-left transition-all hover:shadow-md hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-70"
            >
              {/* 渐变色块 */}
              <div className={`bg-gradient-to-br ${gradient} h-20 w-full flex items-center justify-center`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {activeTemplates.map((template) => (
            ...
            >
              ...
              <div className={`relative bg-gradient-to-br ${template.gradient} aspect-[4/3] w-full overflow-hidden`}>
                {template.imageUrl && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
